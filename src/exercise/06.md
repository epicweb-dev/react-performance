...
types into the `<DogInput />`. As it turns out, the state that the
...
you'll notice that the `Grid` takes quite a bit of time on its own that was
ultimately wasted because there were no actual DOM updates.
