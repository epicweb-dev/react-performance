...
export function makeWorkerApi(worker: Worker) {
	...
	return async function sendWorkerMessage(...args: Array<any>) {
		const messageId = Math.random().toString(16).slice(2)
		const deferred = makeDeferred()
		messageMap.set(messageId, deferred)
		worker.postMessage({ messageId, args })
		return deferred.promise
	}
}

export function makeWorkerMessageHandler(cb: (...args: Array<any>) => any) {
	return async function handleWorkerMessage(event: MessageEvent) {
		if (!event.data) return
		const { messageId, args } = event.data
